...
int main() {
	freopen("ground_cluster_10_1.txt", "r", stdin);
	freopen("out/CCTS_ground_cluster_10_1.txt", "w", stdout);
	...
	while (cin >> temp) {
		...
		if (node_id >= 1 && node_id <= 4 && part_one) {
			LIMIT = 4;
			part_one = false;
			part_two = true;
			TOTAL_ITER = 0;
		} else if (node_id >= 5 && node_id <= 8 && part_two) {
			LIMIT = 4;
			part_two = false;
			part_three = true;
			TOTAL_ITER = 0;
		} else if (node_id >= 9 && part_three) {
			LIMIT = 2;
			part_three = false;
			part_two = false;
			part_one = false;
		...
		if (hash_counter[counter] == LIMIT) {
			TOTAL_ITER++;
			total_iterations = min(total_iterations, TOTAL_ITER);
	...
	cout << "total iter = " << total_iterations << endl;
	...
	while (cin >> temp) {
		...
		if (hash_counter[counter] == LIMIT + 1) {
			TOTAL_ITER++;
			cout << "Iteration = " << TOTAL_ITER << " ";
			for (int ab = 0; ab < LIMIT + 1; ab++) {
				while (Q.size() > LIMIT + 1) {
					Q.pop();
				}
				auto X = Q.front();
				Q.pop();
